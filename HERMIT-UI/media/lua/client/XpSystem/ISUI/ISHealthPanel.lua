...
function ISHealthPanel:render()
    ...
    -- Add 20px padding
    y = y + 20
    self:drawText(getText("IGUI_health_Overall_Body_Status"), self.healthPanel.x + 165, y, 0.85, 0.56, 0.03, 1.0, UIFont.Small)
    y = y + fontHgt
    ...
    self:drawText(str, self.healthPanel.x + 165, y, 1.0, 1.0, 1.0, 1.0, UIFont.Small)
    y = y + fontHgt + 10

    local x = 180;
    local fgBar = {r=0.55, g=0.26, b=0.01, a=0.5}

    local doctor = self.otherPlayer or self.character
    local textX = self.healthPanel.x + 165  -- This is the X position used on line 434
    if doctor:getJoypadBind() ~= -1 then
        -- Draw the joypad button texture at the bottom right
        self:drawTextureScaled(self.abutton, textX, self.height - PAD_BOTTOM - fontHgt, fontHgt, fontHgt, 1.0, 1.0, 1.0, 1.0);
        -- Draw the joypad treatment text next to the button
        self:drawText(getText("IGUI_health_JoypadTreatment"), textX + fontHgt + 2, self.height - PAD_BOTTOM - fontHgt, 1, 1, 1, 1, UIFont.Small);
    else
        -- Draw the mouse treatment text at the bottom right
        self:drawText(getText("IGUI_health_RightClickTreatement"), textX, self.height - PAD_BOTTOM - fontHgt, 1, 1, 1, 1, UIFont.Small);
    end



    -- Center fitness button below the body image panel
    self.fitness:setX(self.bodyPartPanel:getX() + (self.bodyPartPanel:getWidth() - self.fitness:getWidth()) / 2);
    self.fitness:setY(self.bodyPartPanel:getBottom() + 10);
...
function ISHealthBodyPartListBox:doDrawItem(y, item, alt)
    ...
    if item.itemindex == self.selected then
        self:drawRect(0, y, self:getWidth(), item.height, 1, 0.05, 0.05, 0.05);
    ...
    if ISHealthPanel.cheat then
        self:drawText(BodyPartType.getDisplayName(bodyPart:getType()) .. " (health=" .. round(bodyPart:getHealth(), 4) .. ")", x, y, 1, 1, 1, 1, UIFont.Small);
    else
        self:drawText(BodyPartType.getDisplayName(bodyPart:getType()), x, y, 1, 1, 1, 1, UIFont.Small);
    end
    y = y + fontHgt;
    if bodyPartAction then
        self:drawProgressBar(x, y, self.width - 10 - x, fontHgt, bodyPartAction.delta, fgBar)
        self:drawText(bodyPartAction.jobType, x, y, 0.85, 0.56, 0.03, 1, UIFont.Small)
        y = y + fontHgt
    else
        ...
        if actionQueue and actionQueue.queue and healthPanel.actions and healthPanel.actions[actionQueue.queue[1]] == bodyPart then
            ...
            self:drawText(actionQueue.queue[1].jobType, x, y, 0.85, 0.56, 0.03, 1, UIFont.Small); -- jobType is a hack for CraftingUI as well
            y = y + fontHgt
        end
    end
    if bodyPart:getPlantainFactor() > 0 then
        self:drawText("- " .. getText("ContextMenu_PlantainCataplasm"),x, y, 0.16, 0.85, 0.85, 1, UIFont.Small);
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            self:drawText("     - factor " .. round(bodyPart:getPlantainFactor(),2), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
            y = y + fontHgt;
        end
    end
    if bodyPart:getComfreyFactor() > 0 then
        self:drawText("- " .. getText("ContextMenu_ComfreyCataplasm"),x, y, 0.16, 0.85, 0.85, 1, UIFont.Small);
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            self:drawText("     - factor " .. round(bodyPart:getComfreyFactor(),2), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
            y = y + fontHgt;
        end
    end
    if bodyPart:getGarlicFactor() > 0 then
        self:drawText("- " .. getText("ContextMenu_GarlicCataplasm"),x, y, 0.16, 0.85, 0.85, 1, UIFont.Small);
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            self:drawText("     - factor " .. round(bodyPart:getGarlicFactor(),2), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
            y = y + fontHgt;
        end
    end
    if bodyPart:scratched() then
        ...
        self:drawText("- " .. getText("IGUI_health_Scratched") .. text, x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            if(bodyPart:getScratchTime() > 0) then
                self:drawText("     - scratch Time " .. round(bodyPart:getScratchTime(),2), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
                y = y + fontHgt;
            end
        end
    end
    if bodyPart:isCut() then
        ...
        self:drawText("- " .. getText("IGUI_health_Cut") .. text, x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            if(bodyPart:getCutTime() > 0) then
                self:drawText("     - cut Time " .. round(bodyPart:getCutTime(),2), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
                y = y + fontHgt;
            end
        end
    end
    if bodyPart:deepWounded() then
        ...
        self:drawText("- " .. getText("IGUI_health_DeepWound") .. " " .. text, x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            self:drawText("     - deepWound Time " .. round(bodyPart:getDeepWoundTime(),2), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
            y = y + fontHgt;
        end
    end
    if bodyPart:bitten() then
        self:drawText("- " .. getText("IGUI_health_Bitten"), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            self:drawText("     - bite Time " .. round(bodyPart:getBiteTime(),2), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
            y = y + fontHgt;
        end
    end
    if bodyPart:getAdditionalPain() > 10 then
        if bodyPart:getAdditionalPain() > 50 then
            self:drawText("- " .. getText("IGUI_health_HeavyPain"), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
        else
            self:drawText("- " .. getText("IGUI_health_Pain"), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
        end
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            self:drawText("     - additional pain " .. round(bodyPart:getAdditionalPain(),2), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
            y = y + fontHgt;
        end
    end
    if bodyPart:getStiffness() > 5 then
        ...
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            self:drawText("     - exercise fatigue " .. round(bodyPart:getStiffness(),2), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
            y = y + fontHgt;
        end
    end
    if bodyPart:bleeding() then
        self:drawText("- " .. getText("IGUI_health_Bleeding"),x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            self:drawText("     - bleeding Time " .. round(bodyPart:getBleedingTime(),2),x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
            y = y + fontHgt;
        end
    end
    if bodyPart:getFractureTime() > 0 and bodyPart:getSplintFactor() == 0 then
        ...
        self:drawText("- " .. getText("IGUI_health_Fracture") .. " " .. text,x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            self:drawText("     - fracture Time " .. round(bodyPart:getFractureTime(),2), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
            y = y + fontHgt;
        end
    end
    if bodyPart:getSplintFactor() > 0 then
        ...
        self:drawText("- " .. getText("IGUI_health_Splinted") .. " " .. text,x, y, 0.16, 0.85, 0.85, 1, UIFont.Small);
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            self:drawText("     - splint Factor " .. round(bodyPart:getSplintFactor(),2), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
            y = y + fontHgt;
        end
    end
    if bodyPart:bandaged() then
        if bodyPart:getBandageLife() > 0 then
            self:drawText("- " .. getText("IGUI_health_Bandaged"), x, y, 0.16, 0.85, 0.85, 1, UIFont.Small);
        else
            self:drawText("- " .. getText("IGUI_health_DirtyBandage"), x, y, 0.85, 0.56, 0.03, 1, UIFont.Small);
        end
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            if(bodyPart:getBandageLife() > 0) then
                self:drawText("     - Bandage Life " .. round(bodyPart:getBandageLife(), 4), x, y, 0.16, 0.85, 0.85, 1, UIFont.Small);
                y = y + fontHgt;
            end
            if(bodyPart:getBleedingTime() > 0) then
                self:drawText("     - bleeding Time " .. round(bodyPart:getBleedingTime(), 4), x, y, 0.16, 0.85, 0.85, 1, UIFont.Small);
                y = y + fontHgt;
            end
            if(bodyPart:getScratchTime() > 0) then
                self:drawText("     - scratch Time " .. round(bodyPart:getScratchTime(), 4), x, y, 0.16, 0.85, 0.85, 1, UIFont.Small);
                y = y + fontHgt;
            end
            if(bodyPart:getCutTime() > 0) then
                self:drawText("     - cut Time " .. round(bodyPart:getCutTime(), 4), x, y, 0.16, 0.85, 0.85, 1, UIFont.Small);
                y = y + fontHgt;
            end
            if(bodyPart:getDeepWoundTime() > 0) then
                self:drawText("     - deep-wound Time " .. round(bodyPart:getDeepWoundTime(), 4), x, y, 0.16, 0.85, 0.85, 1, UIFont.Small);
                y = y + fontHgt;
            end
        end
    end
    if bodyPart:isInfectedWound() and not bodyPart:bandaged() then
        if healthPanel.doctorLevel > 8 or (bodyPart:getWoundInfectionLevel() * 10 >= (2.5 - healthPanel.doctorLevel)) or ISHealthPanel.cheat then -- only a good doctor will know fast if a wound is infected or not
            self:drawText("- " .. getText("IGUI_health_Infected"), x, y, 0.85, 0.56, 0.03, 1, UIFont.Small);
            y = y + fontHgt;
            if ISHealthPanel.cheat then
                self:drawText("     - WOUND infection Time " .. round(bodyPart:getWoundInfectionLevel(), 4), x, y, 0.85, 0.56, 0.03, 1, UIFont.Small);
                y = y + fontHgt;
            end
        end
    end
    if bodyPart:haveBullet() and not bodyPart:bandaged() then
        self:drawText("- " .. getText("IGUI_health_LodgedBullet"), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
        y = y + fontHgt;
    end
    if bodyPart:getBurnTime() > 0 and not bodyPart:bandaged() then
        ...
        self:drawText("- " .. getText("IGUI_health_Burned") .. burnText, x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            self:drawText("     - burn Time " .. round(bodyPart:getBurnTime(), 4) .. "(" .. bodyPart:getLastTimeBurnWash() .. ")", x, y, 0.85, 0.56, 0.03, 1, UIFont.Small);
            y = y + fontHgt;
        end
    end
    if bodyPart:stitched() then
        ...
        self:drawText("- " .. getText("IGUI_health_Stitched") ..text, x, y, 0.16, 0.85, 0.85, 1, UIFont.Small);
        y = y + fontHgt;
        if ISHealthPanel.cheat then
            self:drawText("     - stitch Time " .. round(bodyPart:getStitchTime(), 4), x, y, 0.16, 0.85, 0.85, 1, UIFont.Small);
            y = y + fontHgt;
        end
    end
    if bodyPart:haveGlass() and not bodyPart:bandaged() then
        self:drawText("- " .. getText("IGUI_health_LodgedGlassShards"), x, y, 0.56, 0.02, 0.02, 1, UIFont.Small);
        y = y + fontHgt;
    end
    y = y + 5;
    return y;

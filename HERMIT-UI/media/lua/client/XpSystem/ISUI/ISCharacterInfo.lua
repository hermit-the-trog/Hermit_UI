...
function ISCharacterInfo:render()
	...
	elseif ISCharacterInfo.timerMultiplierAnim <= 1500 then
        ISCharacterInfo.animOffset = 10;
	elseif ISCharacterInfo.timerMultiplierAnim <= 2000 then
        ISCharacterInfo.animOffset = 20;
	else
		ISCharacterInfo.timerMultiplierAnim = 0;
	end
	...
	local rowHgt = math.max(fontHgt, progressHgt + 3)
	for _,parentPerk in ipairs(sorted) do
		local perkList = self.perks[parentPerk:getType()]
		-- we first draw our parent name
		self:drawText(parentPerk:getName(), left + 20, y, 0.55, 0.26, 0.01, 1, UIFont.Small);
		...
		for ind, perk in ipairs(perkList) do
            ...
            if self.char:getXp():getMultiplier(perk:getType()) > 0 then
                -- Add 2 to each y coordinate for better alignment
                self:drawTexture(self.disabledArrow, left + 30 + self.txtLen, y + 6, 1, 1, 1, 1);
                self:drawTexture(self.disabledArrow, left + 40 + self.txtLen, y + 6, 1, 0.85, 0.56, 0.03);
                self:drawTexture(self.disabledArrow, left + 50 + self.txtLen, y + 6, 1, 0.16, 0.85, 0.85);

                if ISCharacterInfo.animOffset > -1 then
                    -- Also adjust the animated arrow
                    self:drawTexture(self.arrow, left + 30 + self.txtLen + ISCharacterInfo.animOffset, y + 6, 0.3, 1, 1, 1);
                end
            end
...
--~ 	self:drawText("Hauling : " .. getPlayer():getXp():getXP(Perks.Hauling), self.x + 8, y, 1, 1, 1, 1, UIFont.Small);
	...
	if self.joyfocus then
		if self.joypadIndex and self.joypadIndex >= 1 and self.joypadIndex <= #self.progressBars then
			...
			self:drawRectBorder(left-2, bar:getY()-2, (right - left) + 2, bar:getHeight() + 3, 0.85, 0.56, 0.03, 1.0);
			if bar.tooltip then

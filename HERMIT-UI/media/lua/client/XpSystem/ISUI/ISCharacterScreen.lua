...
function ISCharacterScreen:prerender()
	...
	self:drawRectBorder(x - 2, y - 2, w + 4, h + 4, 0.95, 0.0, 0.0, 0.0);
...
--~ 	ISCharacterScreen.loadTraits(self);
	...
	local nameWid = getTextManager():MeasureStringX(UIFont.Large, nameText)
	self:drawText(nameText, nameX, z, 0.16, 0.85, 0.85, 0.95, UIFont.Large);
...
--    self:drawText(round(self.char:getNutrition():getCalories(), 2) .. "", self.xOffset + 10, z, 1,1,1,0.5, UIFont.Small);
    self:drawTextRight(getText("IGUI_char_Weight"), self.xOffset, z, 1, 1, 1, 0.25, UIFont.Small);
    local weightStr = tostring(round(self.char:getNutrition():getWeight(), 0))

    -- Default color (Gold)
    local weightColor = {r = 0.85, g = 0.56, b = 0.16, a = 1}

    -- Change color based on weight change
    if self.char:getNutrition():isIncWeight() or self.char:getNutrition():isIncWeightLot() then
        weightColor = {r = 0.55, g = 0.26, b = 0.01, a = 1} -- Red for increasing weight
    elseif self.char:getNutrition():isDecWeight() then
        weightColor = {r = 0.16, g = 0.85, b = 0.85, a = 1} -- Blue for decreasing weight
    end

    -- Draw weight text aligned with other UI elements
    self:drawText(weightStr, self.xOffset + 10, z, weightColor.r, weightColor.g, weightColor.b, weightColor.a, UIFont.Large);

    -- Draw weight change indicators aligned with text
    if self.char:getNutrition():isIncWeight() or self.char:getNutrition():isIncWeightLot() or self.char:getNutrition():isDecWeight() then
        local nutritionWidth = getTextManager():MeasureStringX(UIFont.Large, weightStr) + 20;
        local iconY = z + (getTextManager():getFontFromEnum(UIFont.Large):getLineHeight() - 16) / 2 -- Center 16px icons vertically

        if self.char:getNutrition():isIncWeight() and not self.char:getNutrition():isIncWeightLot() then
            self:drawTexture(self.weightIncTexture, self.xOffset + nutritionWidth + 1, iconY, 1, 1, 1, 1)
        end
        if self.char:getNutrition():isIncWeightLot() then
            self:drawTexture(self.weightIncLotTexture, self.xOffset + nutritionWidth + 1, iconY, 1, 1, 1, 1)
        end
        if self.char:getNutrition():isDecWeight() then
            self:drawTexture(self.weightDecTexture, self.xOffset + nutritionWidth + 1, iconY, 1, 1, 1, 1)
        end
    end
    ...
    -- Hair style UI elements
    self:drawTextRight(getText("IGUI_char_HairStyle"), self.xOffset, finalY, 1, 1, 1, 0.25, UIFont.Small);
    self:drawText(self.hairStyle, self.xOffset + 10, finalY, 0.92, 0.71, 0.15, 1, UIFont.Small);
	...
	if not self.char:isFemale() then
		self:drawTextRight(getText("IGUI_char_BeardStyle"), self.xOffset, finalY, 1, 1, 1, 0.25, UIFont.Small);
		self:drawText(self.beardStyle, self.xOffset + 10, finalY, 0.92, 0.71, 0.15, 1, UIFont.Small);
...
-- #space for mods# --  flex space for mods -- ?????????????????????

	self.literatureButton:setY(finalY + 16);
	self.literatureButton:setX(self.xOffset); -- Align to the stats
	...
	if #self.traits > 0 then
		self:drawTextRight(getText("IGUI_char_Traits"), self.xOffset, finalY, 1, 1, 1, 0.25, UIFont.Small);
		...
		for i,v in ipairs(self.traits) do
			...
			v:setX(x);
			v:setVisible(true);
			traitBottom = y + v:getTexture():getHeightOrig() + 1
			x = x + v:getTexture():getWidthOrig() + 10;
			if (i < #self.traits) and (x + v:getTexture():getWidthOrig() > self:getWidth() - 20) then
				x = self.xOffset + 10
				y = y + v:getTexture():getHeightOrig() + 10
			end
		end
	...
	z = math.max(finalY, self.avatarY + self.avatarHeight + 25)
	...
	if self.favouriteWeapon then
		self:drawTextRight(getText("IGUI_char_Favourite_Weapon"), x, z, 1, 1, 1, 0.25, UIFont.Small);
		self:drawText(self.favouriteWeapon, x + 10, z, 0.92, 0.71, 0.15, 1, UIFont.Small);
		z = z + smallFontHgt;
	end
	self:drawTextRight(getText("IGUI_char_Zombies_Killed"), x, z, 1, 1, 1, 0.25, UIFont.Small);
	self:drawText(self.char:getZombieKills() .. "", x + 10, z, 0.92, 0.71, 0.15, 1, UIFont.Small);
	z = z + smallFontHgt;
	--[[
	...
	if instanceof(self.char, 'IsoPlayer') and clock and clock:isDateVisible() then
		self:drawTextRight(getText("IGUI_char_Survived_For"), x, z, 1,1,1,0.25, UIFont.Small);
		self:drawText(self.char:getTimeSurvived(), x + 10, z, 0.92, 0.71, 0.15, 1, UIFont.Small);
	end
...
--~ 	self.avatarPanel.render = ISCharacterScreen.drawAvatar;
	...
	self.avatarX = 25
	self.avatarY = 25
	self.avatarWidth = 160
	self.avatarHeight = 349
	...
	self.hairButton.borderColor = {r=0, g=0, b=0, a=0.0};
	...
	self.beardButton.borderColor = {r=0, g=0, b=0, a=0.0};

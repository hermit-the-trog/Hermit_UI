...
function ISGarmentUI:doDrawItem(y, item, alt)
	...
	if item.itemindex == self.selected then
		self:drawRect(0, y, self:getWidth(), item.height, 0.4, 0.55, 0.26, 0.01);
	elseif item.itemindex == self.mouseoverselected then
		self:drawRect(0, y, self:getWidth(), item.height, 0.4, 0.55, 0.26, 0.01);
	end

	self:drawText(part:getDisplayName(), 0, y, 0.9, 0.9, 0.9, 1, UIFont.Small)
...
--				self.listbox.onRightMouseUp = ISGarmentUI.onBodyPartListRightMouseUp
	...
	self:drawText(getText("IGUI_garment_BodyPart"), self.bodyPartColumn, y, 0.85, 0.56, 0.03, 1, UIFont.Small)
	self:drawText(getText("IGUI_health_Scratch"), self.scratchColumn, y, 0.85, 0.56, 0.03, 1, UIFont.Small)
	self:drawText(getText("IGUI_health_Bite"), self.biteColumn, y, 0.85, 0.56, 0.03, 1, UIFont.Small)
	self:drawText(getText("IGUI_health_Bullet"), self.bulletColumn, y, 0.85, 0.56, 0.03, 1, UIFont.Small)
	y = y + (FONT_HGT_SMALL * 2);
	for i,part in ipairs(self.parts) do
		self:drawTexture(self.textures[part:toString()].texture, 5, self.texturesYOffset + 30, 1, 1, 1, 1);
		if self.clothing:getVisual():getHole(part) > 0 and self.textures[part:toString()].hole then
			self:drawTexture(self.textures[part:toString()].hole, 5, self.texturesYOffset + 30, 1, 1, 1, 1);
		end
		if self.clothing:getBloodlevelForPart(part) > 0 and self.textures[part:toString()].blood then
			self:drawTexture(self.textures[part:toString()].blood, 5, self.texturesYOffset + 30, self.clothing:getBloodlevelForPart(part) + 0.1, 1, 1, 1);
		end
		if self.clothing:getPatchType(part) then
			self:drawTexture(self.textures[part:toString()].patch, 5, self.texturesYOffset + 30, 1, 1, 1, 1);
		end
	end
	...
	self:drawText(getText("IGUI_invpanel_Condition"), self.progressX1, y, 0.85, 0.56, 0.03, 1, UIFont.Small)
	self:drawText(getText("IGUI_garment_GlbBlood"), self.progressX2, y, 0.85, 0.56, 0.03, 1, UIFont.Small)
	self:drawText(getText("IGUI_garment_GlbDirt"), self.progressX3, y, 0.85, 0.56, 0.03, 1, UIFont.Small)
	y = self.progressY;
	...
	if not self.clothing:getFabricType() then
		...
		self:drawText(getText("IGUI_garment_CantRepair"), x, 40, 0.55, 0.16, 0.16, 1, UIFont.Small)
	end
end

function ISGarmentUI:drawBar(x, y, width, height, percent, isCondition)
    local color
    if isCondition then
        color = {r=0.16, g=0.85, b=0.85, a=1} -- Blue for condition
    elseif not self.isDirtBar then
        color = {r=0.55, g=0.16, b=0.16, a=1} -- Red for blood
        self.isDirtBar = true
    else
        color = {r=0.55, g=0.26, b=0.01, a=1} -- Brown for dirt
        self.isDirtBar = false
    end
    self:drawProgressBar(x, y, width, height, percent, color)
end
...
function ISGarmentUI:calcProgressPositions()
	local padX = 20
	self.progressX1 = 20      -- Keep first bar position
	self.progressX2 = 180     -- Add ~40px spacing
	self.progressX3 = 340     -- Add ~40px spacing
end
...
function ISGarmentUI:calcColumnWidths()
	...
	self.biteColumn = self.bodyPartColumn + partColumnWidth + 20
	self.scratchColumn = math.max(self.biteColumn + 35 + biteSize)
	self.bulletColumn = math.max(self.scratchColumn + 20 + scratchSize)

	local scrollbarWidth = 17
	local listRight = self.bulletColumn + bulletSize + scrollbarWidth
	local progressWidth = self.progressWidthTotal + 30 * 2
...
function ISGarmentUI:new(x, y, character, clothing)
	...
	o.clothing = clothing;
	o.borderColor = {r=0.0, g=0.0, b=0.0, a=1};

...
function ISTickBox:render()
    ...
    for i,v in ipairs(self.options) do
        ...
        if self:isMouseOver() and (self.mouseOverOption == c) and not self.disabledOptions[self.optionsIndex[self.mouseOverOption]] then
            self:drawRect(self.leftMargin, y+boxDY, self.boxSize, self.boxSize, 1.0, 0.55, 0.26, 0.01);
        else
        ...
        if self.joypadFocused and self.joypadIndex == c then
            self:drawRectBorder(self.leftMargin - 2, y+boxDY - 2, self.width + 2, self.boxSize + 4, 1.0, 0.85, 0.56, 0.06); --gold
            self:drawRect(self.leftMargin, y+boxDY, self.boxSize, self.boxSize, 1.0, 0.85, 0.56, 0.06); --gold
        end
...
function ISTickBox:new (x, y, width, height, name, changeOptionTarget, changeOptionMethod, changeOptionArg1, changeOptionArg2)
    ...
    o.backgroundColor = {r=0.9, g=0.9, b=0.9, a=1.0}; --white
    o.borderColor = {r=0.9, g=0.9, b=0.9, a=1.0}; --white 
    o.choicesColor  = {r=0.85, g=0.56, b=0.03, a=1.0}; --gold
    o.anchorLeft = true;
    o.anchorRight = false;
    o.anchorTop = true;

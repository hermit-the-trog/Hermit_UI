...
--	self:addChild(self.resetBtn);
	...
	-- exercises type
	self.exercises = ISRadioButtons:new(30, 50, 120, 20, self, ISFitnessUI.clickedExe)
	self.exercises.choicesColor = {r=0.85, g=0.56, b=0.03, a=1}
	...
	self.regularityProgressBarWidth = self.exercises:getWidth()

	-- time
	self.timeLbl = ISLabel:new(self.exercises.x, barBottom + 15, FONT_HGT_SMALL, getText("IGUI_FitnessTime"), 0.85,0.56,0.03,1, UIFont.Small, true)
	self.timeLbl:initialise();
	self.timeLbl:instantiate();
	self:addChild(self.timeLbl)

	self.exeTime = ISTextEntryBox:new("10", self.timeLbl.x + self.timeLbl:getWidth() + 20, self.timeLbl.y, 30, FONT_HGT_MEDIUM + 2 * 2)
	...
	self.minusBtn = ISButton:new(self.plusBtn.x + self.plusBtn:getWidth() + 5, self.exeTime.y, self.exeTime:getHeight(), self.exeTime:getHeight(), "-", self, self.onClickTime)
	...
	self.tooltipLbl = ISRichTextPanel:new(self.exercises.x + self.exercises:getWidth() + 10, 
                                     self.exercises.y - 10, -- Subtract 2 pixels to align with radio buttons
                                     self:getWidth() - (self.exercises.x + self.exercises:getWidth()), 
                                     150);
...
--		self:drawText("Current exe Regularity: " .. round(self.currentRegularity, 4), 10, 10, 1,1,1,1, UIFont.Small);
	...
	self:drawProgressBar(self.exercises.x, self.barY, self.regularityProgressBarWidth, 
        self.barHgt, self:getCurrentRegularity() * 1.5, self.fgBar)
    self:drawRectBorder(self.exercises.x, self.barY, self.regularityProgressBarWidth, 
        self.barHgt, 1.0, 0.0, 0.0, 0.0)
    self:drawTextCentre(getText("IGUI_FitnessRegularity"), 
        self.exercises.x + self.regularityProgressBarWidth / 2, self.barY + (self.barHgt - FONT_HGT_SMALL) / 2, 
...
function ISFitnessUI:prerender()
	...
	self:drawTextCentre(getText("ContextMenu_Fitness"), self:getWidth()/2, self.titleY, 0.55,0.26,0.01,1, UIFont.Medium);
...
function ISFitnessUI:new(x, y, width, height, player, clickedSquare)
    ...
    o.borderColor = {r=0, g=0, b=0, a=0.99};
    o.backgroundColor = {r=0, g=0, b=0, a=0.99};
    o.width = width;
    o.height = height;
    o.player = player;
    o.moveWithMouse = true;
    o.buttonBorderColor = {r=0.0, g=0.0, b=0.0, a=0.0};
	o.titleY = 10
	o.fgBar = {r=0,16, g=0.85, b=0.85, a=1 }
	o.fgBarOrange = {r=0.85, g=0.56, b=0.03, a=1 }
	o.fgBarRed = {r=0.55, g=0.16, b=0.16, a=1 }
	o.clickedSquare = clickedSquare;
	o.zoneProgress = 100;
	o.fitness = player:getFitness();
	o.fgBar = {r=0,16, g=0.85, b=0.85, a=1 }
	o.regularityProgressBarWidth = 320;

...
function ISContextMenu:render()
	...
	self:drawRect(0, dy - self:getYScroll(), self.width, self.height - dy * 2, 0.9, 0, 0, 0); 
	self:drawRect(0, dy - self:getYScroll(), iconSize + iconShiftX * 2, self.height - dy * 2, 0.25, 0.1, 0.1, 0.1); 
	self:drawRectBorderStatic(0, dy, self.width, self.height - dy * 2, self.borderColor.a, self.borderColor.r, self.borderColor.g, self.borderColor.b)
	...
	for i,k in ipairs(self.options) do
		...
		elseif self.mouseOver == c then
			if k.notAvailable then
				self:drawRect(0, y, self.width, self.itemHgt, 0.2, 0.56, 0.02, 0.02); --red not available
				self:drawRectBorder(0, y, self.width, self.itemHgt, 0.2, 0.56, 0.02, 0.02); --red
				if k.iconTexture ~= nil then
					self:drawTextureScaledAspect(k.iconTexture, iconShiftX, y + iconShiftY, iconSize, iconSize, 1, 1, 1, 0.8)
				end

				self:drawText(k.name, textForIconShift, y + textDY, 1, 0.92, 0.71, 0.15, self.font); --yellow not available
				if k.subOption ~= nil then 
					self:drawTextRight(">", self.width - 4, y + textDY, 1, 0.92, 0.71, 0.15, self.font); --yellow
				end
			else
				self:drawRect(0, y, self.width, self.itemHgt, 0.8, 0.55, 0.26, 0.01); --brown selected
				self:drawRectBorder(0, y, self.width, self.itemHgt, 0.8, 0.55, 0.26, 0.01    ); --brown
				if k.iconTexture ~= nil then
					self:drawTextureScaledAspect(k.iconTexture, iconShiftX, y + iconShiftY, iconSize, iconSize, 1, 1, 1, 0.8)
				end
...
--~ 			end
			if k.isDefaultOption then
				self:drawText("<", backArrowDX, y + textDY, 1, 1, 1, 1, self.font);
			end
			-- if we have a subOption, we set it visible
			if k.subOption ~= nil then
				self:drawTextRight(">", self.width - 4, y+textDY, 1, 1, 1, 0.8, self.font);
				if self:isOptionSingleMenu() then
					-- nothing
			...
			if k.notAvailable then
				...
				self:drawText(k.name, textForIconShift, y+textDY, 0.8, 0.1, 0.1, 0.5, self.font);

                if k.subOption ~= nil then
                    self:drawTextRight(">", self.width - 4, y+textDY, 0.8, 0.1, 0.1, 0.5, self.font);
                end
            else
				...
				self:drawText(k.name, textForIconShift, y+textDY, 1, 1, 1, 0.8, self.font);

				if k.isDefaultOption then
					self:drawText("<", backArrowDX, y + textDY, 1, 1, 1, 0.8, self.font);
				end
				if k.subOption ~= nil then
					self:drawTextRight(">", self.width - 4, y+textDY, 1, 1, 1, 0.8, self.font);
				end
	...
	if offTop then
		self:drawRect(0, 0 - self:getYScroll(), self.width, self.scrollIndicatorHgt, 0.9, 0.1, 0.1, 0.1);
	...
	if offBottom then
		self:drawRect(0, self.height - self.scrollIndicatorHgt - self:getYScroll(), self.width, self.scrollIndicatorHgt, 0.9, 0.1, 0.1, 0.1);
		local x = self.width / 2 - 14 - 7
...
function ISContextMenu:new (x, y, width, height, zoom)
	...
	o.padTopBottom = 0
	o.borderColor = {r=0.0, g=0.0, b=0.0, a=0.8}; --hermit
	o.backgroundColor = {r=0.0, g=0.0, b=0.0, a=0.8}; --hermit
	o.backgroundColorMouseOver = {r=0.55, g=0.26, b=0.01, a=0.7}; --brown
	o.width = width;
	o.height = height;
	o.anchorLeft = true;
	o.anchorRight = false;

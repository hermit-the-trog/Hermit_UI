...
-- hermit start by replacing this code
...
-- 			self: Border(0, 3, self:getWidth(), self:getHeight()-4, self.borderColor.a, self.borderColor.r, self.borderColor.g, self.borderColor.b)
-- 			if self.doSetStencil then
-- 				self:clearStencilRect()
-- 			end
-- 			if self.doRepaintStencil then
...
function ISScrollBar:render()
    if self.vertical then
        ...
        if sh > self:getHeight() then
            ...
            local fixedBarHeight = 64 -- Set a fixed height for the scrollbar
            self.barx = 4
            self.bary = 16 + self.pos * (self:getHeight() - (16 * 2) - fixedBarHeight)
            self.barwidth = 8
            self.barheight = fixedBarHeight

            self:drawTexture(self.uptex, 1 + 3, 0, 0, 1, 1, 1)
            self:drawTexture(self.downtex, 1 + 3, self.height - 20, 0, 1, 1, 1)
            self:drawTextureScaled(self.midtex, 2 + 3, self.bary + 2, 9, self.barheight - 4, 1, 1, 1, 1)
            self:drawTexture(self.toptex, 2 + 3, self.bary, 0, 1, 1, 1)
            self:drawTexture(self.bottex, 2 + 3, (self.bary + self.barheight) - 3, 0, 1, 1, 1)
            self:drawRectBorder(3, 0, self:getWidth() - 4, self:getHeight(), self.borderColor.a, self.borderColor.r, self.borderColor.g, self.borderColor.b)
        ...
        if sw > self:getWidth() then
            ...
            local fixedBarWidth = 64 -- Set a fixed width for the scrollbar
            self.barx = 16 + self.pos * (self:getWidth() - (16 * 2) - fixedBarWidth)
            self.bary = 4
            self.barwidth = fixedBarWidth
            self.barheight = 8

            self:drawTexture(self.uptex, 0, 1 + 3, 0, 1, 1, 1)
            self:drawTexture(self.downtex, self.width - 20, 1 + 3, 0, 1, 1, 1)
            self:drawTextureScaled(self.midtex, self.barx + 2, 2 + 3, self.barwidth - 4, 9, 1, 1, 1, 1)
            self:drawTexture(self.toptex, self.barx, 2 + 3, 0, 1, 1, 1)
            self:drawTexture(self.bottex, (self.barx + self.barwidth) - 3, 2 + 3, 0, 1, 1, 1)
...
function ISScrollBar:new (parent, vertical)
	...
	setmetatable(o, self)
	self.__index = self
    o.backgroundColor = {r=0.0, g=0.0, b=0.0, a=0.0}; --hermit
    o.borderColor = {r=0.0, g=0.0, b=0.0, a=0.0};  --hermit
    o.anchorLeft = true;
	o.anchorRight = false;
	o.anchorTop = true;

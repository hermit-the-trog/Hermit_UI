...
function ISRichTextPanel:processCommand(command, x, y, lineImageHeight, lineHeight)
    ...
    if command == "H1" then
		...
		self.rgb[self.currentLine].r = 0.55
		self.rgb[self.currentLine].g = 0.26
		self.rgb[self.currentLine].b = 0.01
    ...
    if command == "H2" then
        self.orient[self.currentLine] = "left";
        self.rgb[self.currentLine] = {};
		self.rgb[self.currentLine].r = 0.85
		self.rgb[self.currentLine].g = 0.56
		self.rgb[self.currentLine].b = 0.03
    ...
    if string.find(command, "RED") then
        self.rgb[self.currentLine] = {};
		self.rgb[self.currentLine].r = 0.56
		self.rgb[self.currentLine].g = 0.02
		self.rgb[self.currentLine].b = 0.02
        self.rgbCurrent = self.rgb[self.currentLine]
    end
    ...
    if string.find(command, "GREEN") then
        self.rgb[self.currentLine] = {};
		self.rgb[self.currentLine].r = 0.16
		self.rgb[self.currentLine].g = 0.85
		self.rgb[self.currentLine].b = 0.85
        self.rgbCurrent = self.rgb[self.currentLine]
    end
...
--				self:drawRect(self.lineX[c] + self.marginLeft, self.lineY[c] + self.marginTop, self.width, 1, 1.0, 0.5, 0.5, 0.5)
	...
	if ISRichTextPanel.drawMargins then
		self:drawRectBorder(0, 0, self.width, self:getScrollHeight(), 0.95, 0, 0, 0)
		self:drawRect(self.marginLeft, 0, 1, self:getScrollHeight(), 0.95, 0, 0, 0)
		local maxLineWidth = self.maxLineWidth or (self.width - self.marginRight - self.marginLeft)
		-- self:drawRect(self.marginLeft + maxLineWidth, 0, 1, self:getScrollHeight(), 0.95, 0, 0, 0)
		self:drawRect(self.width - self.marginRight, 0, 1, self:getScrollHeight(), 0.95, 0, 0, 0)
		self:drawRect(0, self.marginTop, self.width, 1, 0.95, 0, 0, 0)
		self:drawRect(0, self:getScrollHeight() - self.marginBottom, self.width, 1, 0.95, 0, 0, 0)
	end

...
function ISButton:prerender()
	if self.displayBackground and not self.isJoypad then
		...
		if self.pressed then
			...
			self.backgroundColorPressed.r = self.backgroundColorMouseOver.r * 0.85
			self.backgroundColorPressed.g = self.backgroundColorMouseOver.g * 0.56
			self.backgroundColorPressed.b = self.backgroundColorMouseOver.b * 0.03
...
--        print("btn:image")
	...
	elseif self.displayBackground and not self.isJoypad and self.joypadFocused then
		...
		self:drawText(self.title, x, (self.height / 2) - (height/2) + self.yoffset, 0.1, 0.1, 0.1, 1, self.font);
	end
	if self.overlayText then
		self:drawTextRight(self.overlayText, self.width, self.height - 10, 1, 1, 1, 1, UIFont.Small);
	end
    ...
    if false and self.mouseOver and self.tooltip then
        self:drawRect(self:getMouseX() + 23, self:getMouseY() + 23, getTextManager():MeasureStringX(UIFont.Small, self.tooltip) + 24, 32+24, 1, 0.55, 0.26, 0.01);
        self:drawRectBorder(self:getMouseX()  + 23, self:getMouseY() + 23, getTextManager():MeasureStringX(UIFont.Small, self.tooltip) + 24, 32+24, 0.8, 0.55, 0.26, 0.01);
...
function ISButton:setEnable(bEnabled)
	...
	if bEnabled then
		...
		self:setBorderRGBA(0.56, 0.16, 0.16, 0.95)
	end
end
...
function ISButton:new (x, y, width, height, title, clicktarget, onclick, onmousedown, allowMouseUpProcessing)
	...
	o.backgroundColorMouseOver = {r=0.85, g=0.56, b=0.03, a=1.0};

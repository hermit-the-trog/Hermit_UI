...
function ISInventoryPane:createChildren()
  ...
  --  self.expandAll.backgroundColorMouseOver.a = 0;
   ...
   -- self.collapseAll.backgroundColorMouseOver.a = 0;
	...
	self.nameHeader.borderColor.a = 0.0;
	...
	self.typeHeader.borderColor.a = 0.0;
    ...
    self.contextButton1.borderColor.a = 0.0;
    ...
    self.contextButton2.borderColor.a = 0.0;
    ...
    self.contextButton3.borderColor.a = 0.0;
...
function ISInventoryPane:rendericons()
    ...
    for i = 0, it:size()-1 do
        local item = it:get(i);
        self:drawTexture(item:getTex(), (xcount * iw) + xpad + 4, (ycount * ih) + ypad + 4, 1, 1, 1, 1); -- White color with full opacity
...
function ISInventoryPane:renderdetails(doDragged)
    ...
    if not doDragged then
        ...
        self:drawRectStatic(0, 0, self.column2, self.height, 0.95, 0, 0, 0); -- Black color with 95% opacity
    end
    local y = 0;
    local alt = false;
...
--                    if self.joyselection >= #self.itemslist then self.joyselection = 0; end
                    if self.joyselection == y then
                        self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self:getWidth()-1, self.itemHgt, 0.4, 0.55, 0.26, 0.01); -- (a, r, g, b) joypad/gamepad selected item highlight color
                    end
                end
               -- print("trace:e");
               ...
               -- print("trace:g");
                ...
                if self.selected[y+1] ~= nil and not self.highlightItem then -- clicked/dragged item
                    if checkDraggedItems and self.draggedItems:cannotDropItem(item) then
                        self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self:getWidth()-1, self.itemHgt, 0.20, 1.0, 0.0, 0.0);  -- Red with 20% opacity
                    elseif false and (((instanceof(item,"Food") or instanceof(item,"DrainableComboItem")) and item:getHeat() ~= 1) or item:getItemHeat() ~= 1) then
                        if (((instanceof(item,"Food") or instanceof(item,"DrainableComboItem")) and item:getHeat() > 1) or item:getItemHeat() > 1) then
                            self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt,  0.5, math.abs(item:getInvHeat()), 0.0, 0.0);  -- Red with 50% opacity
                        else
                            self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt,  0.5, 0.0, 0.0, math.abs(item:getInvHeat()));  -- Blue with 50% opacity
                        end
                    else
                        self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self:getWidth()-1, self.itemHgt, 0.20, 1.0, 1.0, 1.0);  -- White with 20% opacity
                    end
                elseif self.mouseOverOption == y+1 and not self.highlightItem then -- called when you mouse over an element
                    if(((instanceof(item,"Food") or instanceof(item,"DrainableComboItem")) and item:getHeat() ~= 1) or item:getItemHeat() ~= 1) then
                        if (((instanceof(item,"Food") or instanceof(item,"DrainableComboItem")) and item:getHeat() > 1) or item:getItemHeat() > 1) then
                            self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt,  0.3, math.abs(item:getInvHeat()), 0.0, 0.0);  -- Red with 30% opacity
                        else
                            self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt,  0.3, 0.0, 0.0, math.abs(item:getInvHeat()));  -- Blue with 30% opacity
                        end
                    else
                        self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self:getWidth()-1, self.itemHgt, 0.05, 1.0, 1.0, 1.0);  -- White with 5% opacity
                    end
                else
                    if count == 1 then -- normal background (no selected, no dragging..)
                        ...
                        if self.highlightItem and self.highlightItem == item:getType() then
                            ...
                            self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt,  self.blinkAlpha, 1, 1, 1);  -- White with blinking opacity
                            ...
                            if (((instanceof(item,"Food") or instanceof(item,"DrainableComboItem")) and item:getHeat() ~= 1) or item:getItemHeat() ~= 1) then
                                if (((instanceof(item,"Food") or instanceof(item,"DrainableComboItem")) and item:getHeat() > 1) or item:getItemHeat() > 1) then
                                    if alt then
                                        self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt,  0.15, math.abs(item:getInvHeat()), 0.0, 0.0);  -- Red with 15% opacity
                                    else
                                        self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt,  0.2, math.abs(item:getInvHeat()), 0.0, 0.0);  -- Red with 20% opacity
                                    end
                                else
                                    if alt then
                                        self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt,  0.15, 0.0, 0.0, math.abs(item:getInvHeat()));  -- Blue with 15% opacity
                                    else
                                        self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt,  0.2, 0.0, 0.0, math.abs(item:getInvHeat()));  -- Blue with 20% opacity
                                    end
                                end
                            else
                                if alt then
                                    self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt, 0.02, 1.0, 1.0, 1.0);  -- White with 2% opacity
                                else
                                    self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self.column4, self.itemHgt, 0.2, 0.0, 0.0, 0.0);   -- Black with 20% opacity
                                end
                            end
                        end
                    else
                        if (((instanceof(item,"Food") or instanceof(item,"DrainableComboItem")) and item:getHeat() ~= 1) or item:getItemHeat() ~= 1) then
                            ...
                            self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, self:getWidth()-1, self.itemHgt,  0.4, 0.0, 0.0, 0.0);  -- Black with 40% opacity
                        end
                    end
                end
               -- print("trace:h");
                ...
                if v.equipped then
                    if not doDragged and not equippedLine and y > 0 then
                        self:drawRect(1, ((y+1)*self.itemHgt)+self.headerHgt-1-self.itemHgt, self.column4, 1, 0.4, 0.92, 0.71, 0.41); -- Light brown with 40% opacity, applied to draw a divider between equipped and unequipped items
                    end
                    equippedLine = true
                end
                ...
                if item:getJobDelta() > 0 and (count > 1 or self.collapsed[v.name]) then
                    ...
                    self:drawRect(1+xoff, (y*self.itemHgt)+self.headerHgt+yoff, displayWid * item:getJobDelta(), self.itemHgt, 0.2, 0.4, 1.0, 0.3); -- Light blue with 20% opacity, applied to draw a progress bar for job completion
                end
               -- print("trace:i");
                ...
                if count == 1 then
					...
					-- if we're dragging something and want to put it in a container which is full
					if doDragged and ISMouseDrag.dragging and #ISMouseDrag.dragging > 0 then
						local red = false;
						if red then
							if v.count > 2 then
								self:drawText(itemName.." ("..(v.count-1)..")", self.column2+8+xoff, (y*self.itemHgt)+self.headerHgt+textDY+yoff, 0.56, 0.02, 0.02, 1.0, self.font); -- Red, applied to draw text for item name
							else
								self:drawText(itemName, self.column2+8+xoff, (y*self.itemHgt)+self.headerHgt+textDY+yoff, 0.56, 0.02, 0.02, 1.0, self.font); -- Red, applied to draw text for item name
							end
						else
							if v.count > 2 then
								self:drawText(itemName.." ("..(v.count-1)..")", self.column2+8+xoff, (y*self.itemHgt)+self.headerHgt+textDY+yoff, 0.7, 0.7, 0.7, 1.0, self.font); -- Gray, applied to draw text for item name
							else
								self:drawText(itemName, self.column2+8+xoff, (y*self.itemHgt)+self.headerHgt+textDY+yoff, 0.7, 0.7, 0.7, 1.0, self.font); -- Gray, applied to draw text for item name
							end
						end
					else
               ...
               -- print("trace:j");
                ...
                --~ 							self:drawText(item:getName(), self.column2+8+xoff, (y*18)+16+1+yoff, 0.95, 0.13, 0.13, 1.0);
                    --~ 						else

                if item:getJobDelta() > 0  then
                    if  (count > 1 or self.collapsed[v.name]) then
						if self.dragging == count then
							self:drawText(item:getJobType(), self.column3+8+xoff, (y*self.itemHgt)+self.headerHgt+textDY+yoff, 0.56, 0.02, 0.02, 1.0, self.font); -- Red, applied to draw text for job type
						else
							self:drawText(item:getJobType(), self.column3+8+xoff, (y*self.itemHgt)+self.headerHgt+textDY+yoff, 0.8, 0.8, 0.8, 1.0, self.font); -- Gray, applied to draw text for job type
						end
                    end
                    ...
                    if count == 1 then
                        ...
                        elseif item:getDisplayCategory() then -- display the custom category set in items.txt
                            self:drawText(getText("IGUI_ItemCat_" .. item:getDisplayCategory()), self.column3+8+xoff, (y*self.itemHgt)+self.headerHgt+textDY+yoff, 0.85, 0.56, 0.03, 0.8); --set category text color to gold
                        else
                            self:drawText(getText("IGUI_ItemCat_" .. item:getCategory()), self.column3+8+xoff, (y*self.itemHgt)+self.headerHgt+textDY+yoff, 0.85, 0.56, 0.03, 0.8); --set category text color to gold
                        end
                    else
                        local redDetail = false;
	...
	if self.draggingMarquis then
		...
		self:drawRectBorder(self.draggingMarquisX, self.draggingMarquisY, w, h, 0.8, 0.4, 0.7, 0.9); -- Pink with 80% opacity
    end


    if not doDragged then
		self:drawRectStatic(1, 0, self.width-2, self.headerHgt, 1, 0, 0, 0); -- Black color with 100% opacity
    end
...
function ISInventoryPane:drawProgressBar(x, y, w, h, f, fg)
	...
	local bg = {r=0.0, g=0.0, b=0.0, a=0.0}; -- Black with 0% opacity
...
--~ 	print(red);
	...
	local fgText = {r=0.92, g=0.71, b=0.15, a=0.8} -- Gold with 80% opacity
	if red then fgText = {0.56, 0.02, 0.02, 1.0}  -- Red
        end
	if instanceof(item, "HandWeapon") then
...
function ISInventoryPane:render()
	...
	if resize then
		...
		self:drawRectStatic(self.nameHeader:getRight() - 1, self.nameHeader.y, 2, self.height, 0.5, 1, 1, 1) -- White with 50% opacity
	end
end
...
function ISInventoryPane:new (x, y, width, height, inventory, zoom)
	...
	o.y = y;
    -- Basic UI colors
    o.borderColor = {r=0.0, g=0.0, b=0.0, a=0.9};  -- Black with 90% opacity
    o.backgroundColor = {r=0, g=0, b=0, a=0.9};     -- Black with 90% opacity
	o.width = width;
	o.height = height;
	o.anchorLeft = true;
	o.anchorRight = false;

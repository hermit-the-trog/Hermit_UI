...
function ISGameSoundVolumeControl:getVolumeAtX(x)
    local padX = PADDING_X
    local oneTenth = math.floor((self:getWidth() - padX * 2) / (MAX_VOLUME / VOLUME_STEP))
    padX = padX + (self:getWidth() - padX * 2 - oneTenth * (MAX_VOLUME / VOLUME_STEP)) / 2
    local volume = math.floor(((x - padX) + oneTenth / 2) / oneTenth)
    volume = volume * VOLUME_STEP
    if volume < MIN_VOLUME then return MIN_VOLUME end
    if volume > MAX_VOLUME then return MAX_VOLUME end
    return volume
end

...
function ISInventoryPage:render()
    ...
    if self.joyfocus then
        self:drawRectBorder(0, 0, self:getWidth(), self:getHeight(), 0.0, 0.0, 0.0, 0.0);
...
function ISInventoryPage:addContainerButton(container, texture, name, tooltip)
	...
	button:setBackgroundRGBA(0.0, 0.0, 0.0, 0.95)
	button:setBackgroundColorMouseOverRGBA(0.55, 0.26, 0.01, 1)
	button:setBorderRGBA(0.0, 0.0, 0.0, 0.95)
...
function ISInventoryPage:refreshBackpacks()
	...
	for k,containerButton in ipairs(self.backpacks) do
		if containerButton.inventory == self.inventory then
			...
			containerButton:setBackgroundRGBA(0.55, 0.26, 0.01, 1)
			self.title = containerButton.name
		else
			containerButton:setBackgroundRGBA(0.0, 0.0, 0.0, 0.0)
		end
...
function ISInventoryPage:new (x, y, width, height, inventory, onCharacter, zoom)
    ...
    o.anchorTop = true;
    o.anchorBottom = true;
	o.borderColor = {r=0.0, g=0.0, b=0.0, a=0.9};

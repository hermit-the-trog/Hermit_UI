...
function HorizontalLine:render()
	self:drawRect(0, 0, self.width, 2, 1.0, 0.55, 0.26, 0.01)
end
...
function MainPanel:render()
    ...
    if self.joyfocus then
        self:drawRectBorderStatic(0, 0, self:getWidth(), self:getHeight(), 0.95, 0.85, 0.56, 0.03);
        -- self:drawRectBorderStatic(1, 1, self:getWidth()-2, self:getHeight()-2, 0.95, 0.85, 0.56, 0.03);
    end
end
...
function MainPanel:renderActiveMods()
    ...
    for i=1,activeMods:getMods():size() do
        ...
        if modInfo then
            ...
            self:drawText(modID, x + xoffset, y, 0.55, 0.16, 0.16, 1, UIFont.Small)
        end
        y = y + FONT_HGT_SMALL
    end
...
--    self.survival = survival;
    ...
    local playstyle = ISLabel:new(x + xoffset, y, FONT_HGT_LARGE, getText("UI_NewGame_PlayStyle"), 0.85, 0.56, 0.03, 1, UIFont.NewLarge, true);
    ...
    local survivorDesc = ISLabel:new(survivor:getRight(), y, mediumFontHgt, " - " .. getText("UI_NewGame_Apocalypse_desc"), 0.3, 0.3, 0.3, 1, UIFont.Small, true);
    ...
    local fighterDesc = ISLabel:new(fighter:getRight(), y, mediumFontHgt, " - " .. getText("UI_NewGame_Survivor_desc"), 0.3, 0.3, 0.3, 1, UIFont.Small, true);
    ...
    local builderDesc = ISLabel:new(builder:getRight(), y, mediumFontHgt, " - " .. getText("UI_NewGame_Builder_desc"), 0.3, 0.3, 0.3, 1, UIFont.Small, true);
    ...
    local sandboxDesc = ISLabel:new(sandbox:getRight(), y, mediumFontHgt, " - " .. getText("UI_NewGame_Sandbox_desc2"), 0.3, 0.3, 0.3, 1, UIFont.Small, true);
...
--    y = y + math.max(30, largeFontHgt)
    ...
    local challenges = ISLabel:new(x+xoffset, y, FONT_HGT_LARGE, getText("UI_NewGame_Challenges"), 0.16, 0.85, 0.85, 1, UIFont.NewLarge, true);
    ...
    local mods = ISLabel:new(x+xoffset, y, FONT_HGT_LARGE, getText("UI_NewGame_Mods"), 0.85, 0.56, 0.03, 1, UIFont.NewLarge, true);
    mods:initialise();
...
--    self.joypadIndex = 1
	...
	self.backButton.borderColor = {r=0.0, g=0.0, b=0.0, a=0.0};
...
--	self:insertNewLineOfButtons(self.vehiclesButton)
	...
	self.newButton.borderColor = {r=0.0, g=0.0, b=0.0, a=0.0};
...
function NewGameScreen:prerenderBottomPanelLabel()
    ...
    if NewGameScreen.instance.selectedItem == self or NewGameScreen.instance.selectedDifficulty == self then
        self:drawRect(0 - padLeft, 0, self:getWidth() + padLeft + padRight, self:getHeight(), alpha, 0.0, 0.0, 0.0)
        if self.joypadFocused then
            self:drawRectBorder(0 - padLeft, 0, self:getWidth() + padLeft + padRight, self:getHeight(), 0.9, 0.85, 0.56, 0.03)
        else
            self:drawRectBorder(0 - padLeft, 0, self:getWidth() + padLeft + padRight, self:getHeight(), 0.9, 0.85, 0.56, 0.03)
        end
    elseif self.fadeOut or self.fadeIn then
        ...
        self:drawRect(0 - padLeft, 0, self:getWidth() + padLeft + padRight, self:getHeight(), alpha, 0.85, 0.56, 0.03)
    end
...
function NewGameScreen:render()
    ...
    self:drawRectBorder( self.richText:getX(), self.richText:getY(), self.richText:getWidth(), self.richText:getHeight(), 0.0, 0, 0, 0);
end

...
function WorldSelect:render()
	...
	self:drawRectBorder(self.listbox:getX(), self.listbox:getY(), self.listbox:getWidth(), self.listbox:getHeight(), 0.85, 0.56, 0.03, 1)
	-- self:drawRectBorder(self.mapListbox:getX(), self.mapListbox:getY(), self.mapListbox:getWidth(), self.mapListbox:getHeight(), 0.85, 0.56, 0.03, 1)
end
...
function WorldSelect:doDrawItem(y, item, alt)
	if self.itemsSelectable then
		...
		if self.selected == item.index then
			self:drawRect(0, (y), self:getWidth(), item.height-1, 0.55, 0.26, 0.01, 1)
		elseif isMouseOver then
			self:drawRect(1, y + 1, self:getWidth() - 2, item.height - 2, 0.85, 0.56, 0.03, 1)
		end
	end
...
function WorldSelect:create()
	...
	self.mapListbox.backgroundColor  = {r=0, g=0, b=0, a=0.95}

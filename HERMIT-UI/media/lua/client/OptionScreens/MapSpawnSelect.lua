...
function MapSpawnSelect:render()
	...
	if selectedItem.worldimage ~= nil then
		...
		self:drawRectBorder( BreakPoint, self.startY, selectedItem.worldimage:getWidth(), selectedItem.worldimage:getHeight(), 0.0, 0, 0, 0);
	...
	self:drawRectBorder( self.richText.x, self.richText.y, self.richText:getWidth(), self.richText:getHeight(), 0.0, 0, 0, 0);
end
...
function MapSpawnSelect:doDrawItem(y, item, alt)
	...
	if self.selected == item.index then
		self:drawRect(0, (y), self:getWidth(), item.height-1, 1, 0.55, 0.26, 0.01)
	elseif isMouseOver then
		self:drawRect(1, y + 1, self:getWidth() - 2, item.height - 2, 0.95, 0.55, 0.26, 0.01);
	end
	self:drawRectBorder(0, (y), self:getWidth(), item.height, 0.0, self.borderColor.r, self.borderColor.g, self.borderColor.b)
	local fontHgt = getTextManager():getFontFromEnum(UIFont.Large):getLineHeight()
	local textY = y + (item.height - fontHgt) / 2
	self:drawText(item.text, 15, textY, 1, 1, 1, 1, UIFont.Large)
	y = y + item.height
	return y
end
...
function MapSpawnSelect:create()
	...
	self.textEntryLabel = ISLabel:new(20, 48, 50, getText("UI_mapselecter_savename"), 0.55, 0.26, 0.01, 1, UIFont.Medium, true);
...
--	self:addChild(self.listbox)
	...
	self.listbox.doDrawItem = MapSpawnSelect.doDrawItem
	self.listbox:setOnMouseDoubleClick(self, MapSpawnSelect.onDblClick)
	self.listbox.backgroundColor  = {r=0, g=0, b=0, a=0.95};
	...
	self.richText.backgroundColor  = {r=0, g=0, b=0, a=0.95};
	...
	self.backButton.borderColor = {r=0, g=0, b=0, a=0}

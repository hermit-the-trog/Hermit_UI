...
function SaveInfoPanel:render()
	...
	if self.joyfocus then
		...
		-- self:drawRectBorder(1, 1-self:getYScroll(), self:getWidth()-2, self:getHeight()-2, 1, 0.55, 0.26, 0.01);
	else
		self:drawRectBorderStatic(0, 0, self.width, self.height, 0.0, 0, 0, 0)
	end
end
...
function ConfigPanel:prerender()
	...
	if self:validateSavefileName(self.renameSavefileEntry:getText()) then
		self.renameSavefileEntry.borderColor.r = 0.0
		...
		self.renameSavefileEntry.borderColor.r = 0.55
		self.renameSavefileEntry.borderColor.g = 0.16
		self.renameSavefileEntry.borderColor.b = 0.16
...
function ConfigPanel:render()
	...
	if self.joyfocus then
		self:drawRectBorder(0, -self:getYScroll(), self:getWidth(), self:getHeight(), 0.95, 0.85, 0.56, 0.03);
		-- self:drawRectBorder(1, 1-self:getYScroll(), self:getWidth()-2, self:getHeight()-2, 0.85, 0.56, 0.03, 0.95);
	end
...
--	self.listbox:setAnchorRight(true);
	...
	self.backButton.borderColor = {r=0.1, g=0.1, b=0.1, a=0.95};
	...
	self.playButton.borderColor = {r=0.1, g=0.1, b=0.1, a=0.95};
	...
	self.configButton.borderColor = {r=0.1, g=0.1, b=0.1, a=0.95};
	...
	self.deleteButton.borderColor = {r=0.1, g=0.1, b=0.1, a=0.95};
...
function LoadGameScreen:drawMap(y, item, alt)
	...
	if self.selected == item.index then
		self:drawRect(0, (y), self:getWidth(), self.itemheight-1, 0.95, 0.55, 0.26, 0.01);
	elseif isMouseOver then
		self:drawRect(1, y + 1, self:getWidth() - 2, item.height - 2, 0.95, 0.55, 0.26, 0.01);
    end
    ...
    self:drawRectBorder(0, (y), self:getWidth(), self.itemheight-1, 0.0, self.borderColor.r, self.borderColor.g, self.borderColor.b);
